...
exports.handler = async (req, res) => {
    ...
    if (user_name) {
        ...
        const nameRegex = /^[^\s][a-zA-ZáéíóúñÁÉÍÓÚ\s]{8,38}[^\s]$/;
    ...
    if (user_phone) {
        ...
        if (!phoneRegex.test(user_phone)) {
            valErrs.push({ user_phone: 'Invalid phone. It should contain only numbers and have between 6 and 10 digits.' });
        }
    }
    ...
    if (number_document) {
        ...
        if (!numberDocumentRegex.test(number_document)) {
            valErrs.push({ number_document: 'Invalid number document. It should contain only numbers and have between 6 and 10 digits.' });
        }
    }
    ...
    if (user_username) {
        ...
        const usernameRegex = /^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ\s]{6,90}$/;
