...
@Component({
  selector: 'app-finish-courses',
  templateUrl: './finish-courses.component.html',
  styleUrls: ['./finish-courses.component.css'],
  standalone: true,
  imports: [NgFor, GlobalButtonsComponent],
})
export class FinishCoursesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.suscription = this.apprenticeService
      .getNotificationLearner('get_completed_courses')
      .subscribe((res) => {
        if (!res) return;
        this.dataCourse = res;
      });
